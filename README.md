...
|-----------|-------------|
| `plugins/inventory/aap_gateway.py` | Dynamic inventory - discovers AAP nodes via Gateway API |
| `inventory/aap.yml` | Static inventory - generated by discovery playbook |

#### Dynamic vs Static Inventory

**Dynamic** (`plugins/inventory/aap_gateway.py`):
- Queries Gateway API at runtime
- Always current, no maintenance
- Requires API access each run

**Static** (`inventory/aap.yml`):
- Generated by `discover-aap-inventory.yml`
- Faster execution, no API calls
- Use as inventory source in AAP
